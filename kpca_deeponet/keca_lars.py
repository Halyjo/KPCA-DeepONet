...
class keca():
    ...
    def fit(self, X, n_components):
        """
        Perform Kernel Entropy Component Analysis (KECA) on the input data X.
    
        :param X: Input data (n_samples, n_features)
        :param n_components: Number of dominant entropy modes to retain
        """
        ...
        # Compute Shannon entropy values for eigenvalues
        entropy_values = -eigenvalues * np.log(eigenvalues + 1e-10)
        
        # Sort by entropy values in descending order
        idx = np.argsort(entropy_values)[::-1]
